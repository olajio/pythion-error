...
from modules.elastic.monitor_setup import DeleteMonitorHandler
from helpers.general.notify_teams import notify_hosts_removed

class RemoveMonitoringAction(BaseAction):
    ...
    def execute_action(self):
        # Set up clients
        if not self.elk_key:
            elk_client = DeleteMonitorHandler()
        else:
            elk_client = DeleteMonitorHandler(api_key=self.elk_key)
