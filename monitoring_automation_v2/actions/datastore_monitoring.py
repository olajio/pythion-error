...
from modules.elastic.monitor_setup import DatastoreHandler
from helpers.general.notify_teams import notify_hosts_added

class DatastoreMonitoringAction(BaseAction):
    ...
    def execute_action(self):
        if not self.elk_key:
            elk_client = DatastoreHandler()
        else:
            elk_client = DatastoreHandler(api_key=self.elk_key)
